...
export const ManuscriptView: React.FC = () => {
    ...
    const [condition, setCondition] = useState(0.45); // MS Condition
                ...
                <div className="flex items-center gap-10">
                    ...
                    <div className="flex gap-4">
                        {fragments.map((f, i) => (
                            <button 
                              key={f.id}
                              onClick={() => setCurrentFragmentIndex(i)}
                              className={`px-4 py-2 border-b-2 text-[10px] font-bold uppercase tracking-widest transition-all ${currentFragmentIndex === i ? 'border-amber-500 text-amber-500 bg-amber-500/5' : 'border-transparent text-zinc-500 hover:text-zinc-300'}`}
                            >
                                {f.id}
                            </button>
                        ))}
                ...
                <div className="w-80 flex flex-col gap-6 h-full py-12">
                    <div className="bg-[#2a1d18]/80 p-6 border border-[#3d2c25] flex flex-col gap-4 shadow-xl backdrop-blur-md">
                        <h4 className="text-[10px] uppercase font-black text-zinc-500 tracking-[0.2em] mb-2 flex items-center gap-2">
                           <Eraser size={14} className="text-amber-500" /> Restoration Tools
                        </h4>
                        
                        <button 
                          disabled={isRestoring || condition >= 1.0}
                          onClick={handleRestore}
                          className="w-full bg-[#e8dcc4] text-[#1a1412] p-4 font-black text-xs uppercase tracking-[0.2em] hover:bg-white transition-all shadow-lg flex items-center justify-between group disabled:opacity-20 translate-y-0 active:translate-y-1"
                        >
                        ...
                        <div className="text-[9px] text-[#e8dcc4]/40 italic leading-relaxed">
                            "The Ink requires the sympathetical heat of the athanor to return to the surface of the parchment."
                        </div>
                    </div>

                    <div className="bg-amber-900/10 p-6 border border-amber-500/20 flex-1 flex flex-col gap-4 italic font-serif text-[#e8dcc4]/60">
                         <div className="text-[10px] uppercase font-black text-amber-500 not-italic tracking-widest mb-2">Findings</div>
                         <div className="space-y-4">
                            {condition > 0.9 ? (
                                <div className="p-3 bg-amber-500/5 border border-amber-500/20 text-xs">
                                   Keyword discovered: <b className="text-white">SYMPATHY</b>. 
                                   <p className="mt-1 text-[10px] opacity-60">Used to build Weapon Salve recipes.</p>
                                </div>
                            ) : (
                                <div className="text-xs opacity-40">Restore condition to reveal textual truth...</div>
                            )}
                         </div>
                    </div>

                    <button 
                      className="w-full py-3 border border-zinc-800 text-[10px] font-black uppercase tracking-widest text-zinc-500 hover:text-white transition-all flex items-center justify-center gap-2"
                    >
                       <Save size={14} /> Commit Reconstruction
                    </button>
                </div>
            </main>
